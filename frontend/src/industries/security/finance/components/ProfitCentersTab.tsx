...
import {
  fetchProfitCenters,
  fetchProfitCenterTree,
  createProfitCenter,
  updateProfitCenter,
} from '../api';
import type { ProfitCenter } from '../api';

export const ProfitCentersTab: React.FC = () => {
  const [loading, setLoading] = useState<boolean>(true);
  const [profitCenters, setProfitCenters] = useState<ProfitCenter[]>([]);
  const [treeData, setTreeData] = useState<ProfitCenter[]>([]);
  ...
  const handleOpenModal = (pc?: ProfitCenter) => {
    setErrorMsg(null);
    if (pc) {
      setEditingPC(pc);
      setFormData({
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingPC) {
        await updateProfitCenter(editingPC.id, formData);
      } else {
      ...
      setErrorMsg(err?.response?.data?.detail || JSON.stringify(err?.response?.data) || 'Failed to save profit center.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const filteredProfitCenters = profitCenters.filter((pc) => {
    ...
    return pc.code.toLowerCase().includes(q) || pc.name.toLowerCase().includes(q) || pc.description?.toLowerCase().includes(q);
  });

  const renderTreeNode = (node: ProfitCenter, depth: number = 0) => {
      ...
      <div key={node.id} style={{ marginBottom: '6px' }}>
        <div
          style={{
            ...
            borderLeft: `${3 + depth * 2}px solid #a855f7`,
            marginLeft: `${depth * 22}px`,
            borderBottom: '1px solid rgba(255,255,255,0.03)',
          }}
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <span style={{ fontFamily: 'monospace', fontWeight: 700, color: '#c084fc', fontSize: '13px' }}>
              {node.code}
            </span>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
      ...
      >
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          <input
            type="text"
            placeholder="Search profit center code or title..."
            value={searchQuery}
          ...
          <div style={{ display: 'flex', borderRadius: '6px', overflow: 'hidden', border: '1px solid #334155' }}>
            <button
              ...
              style={{
                padding: '6px 12px',
                border: 'none',
                background: viewMode === 'tree' ? '#a855f7' : '#0f172a',
                color: '#f8fafc',
                fontSize: '12px',
                fontWeight: 600,
                cursor: 'pointer',
            ...
            <button
              ...
              style={{
                padding: '6px 12px',
                border: 'none',
                background: viewMode === 'flat' ? '#a855f7' : '#0f172a',
                color: '#f8fafc',
                fontSize: '12px',
                fontWeight: 600,
                cursor: 'pointer',
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '40px', color: '#94a3b8' }}>Loading profit centers...</div>
        ...
        >
          {treeData.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '30px', color: '#94a3b8' }}>No profit centers defined.</div>
          ) : (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
            <thead>
              <tr style={{ background: 'rgba(0,0,0,0.2)', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                ...
                <th style={{ padding: '12px 16px', textAlign: 'left', color: '#94a3b8' }}>Business Unit</th>
            ...
            <tbody>
              {filteredProfitCenters.length === 0 ? (
                ...
                filteredProfitCenters.map((pc) => (
                  <tr key={pc.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                    <td style={{ padding: '10px 16px', fontFamily: 'monospace', fontWeight: 600, color: '#c084fc' }}>
                      {pc.code}
                    </td>
                    <td style={{ padding: '10px 16px', color: '#f8fafc', fontWeight: 500 }}>{pc.name}</td>
                    <td style={{ padding: '10px 16px', color: '#94a3b8' }}>
                      {pc.parent_code ? `${pc.parent_code} - ${pc.parent_name}` : '—'}
                    </td>
                    <td style={{ padding: '10px 16px', color: '#cbd5e1' }}>{pc.description || '—'}</td>
                    <td style={{ padding: '10px 16px', textAlign: 'center' }}>
                      <span
                        style={{
                          ...
                          padding: '2px 6px',
                          borderRadius: '4px',
                          background: pc.is_active ? 'rgba(34, 197, 94, 0.15)' : 'rgba(239, 68, 68, 0.15)',
                          color: pc.is_active ? '#4ade80' : '#f87171',
                        }}
                      >
        ...
        >
          ...
          >
            <h3 style={{ margin: '0 0 16px 0', fontSize: '16px', color: '#f8fafc' }}>
              {editingPC ? `Edit Profit Center: ${editingPC.code}` : 'New Profit Center'}
            </h3>
            ...
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div>
                ...
                >
                  ...
                  {profitCenters
                    .filter((p) => !editingPC || p.id !== editingPC.id)
                    .map((p) => (
