...
export const PurchaseOrderEditor: React.FC<Props> = ({ onClose, onSuccess, items, warehouses, suppliers, documents }) => {
    ...
    const sourceDocs = documents.filter(d => (d.document_type === 'PURCHASE_REQUEST' && d.status === 'APPROVED') || d.document_type === 'QUOTATION');
    ...
    const handleParentChange = async (docId: string) => {
        ...
        try {
            ...
            console.error("Failed to fetch source document lines", e);
        }
    };
    ...
    const handleSubmit = async (submitApproval = false) => {
        ...
        try {
            ...
            setErrorMsg(err.response?.data?.detail || JSON.stringify(err.response?.data) || 'Failed to create PO.');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div style={{ backgroundColor: 'var(--color-surface)', padding: '24px', borderRadius: '8px', width: '1000px', maxWidth: '95vw', maxHeight: '90vh', overflowY: 'auto' }}>
            <h2 style={{ marginTop: 0, marginBottom: '24px', borderBottom: '1px solid var(--color-border)', paddingBottom: '12px' }}>Create Purchase Order</h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px', marginBottom: '24px' }}>
                <div>
                    <label style={{ display: 'block', marginBottom: '8px', fontSize: '0.9rem', fontWeight: 600 }}>Source PR/Quotation</label>
                ...
                <div>
                    <label style={{ display: 'block', marginBottom: '8px', fontSize: '0.9rem', fontWeight: 600 }}>PO Date *</label>
                ...
                <div>
                    <label style={{ display: 'block', marginBottom: '8px', fontSize: '0.9rem', fontWeight: 600 }}>Delivery Date</label>
                ...
                <div>
                    ...
                    <input type="text" placeholder="e.g. USD, EUR" value={currency} onChange={e => setCurrency(e.target.value)} style={{ width: '100%', padding: '8px', border: '1px solid var(--color-border)', borderRadius: '4px' }} />
                </div>
                <div style={{ gridColumn: 'span 3' }}>
                    <label style={{ display: 'block', marginBottom: '8px', fontSize: '0.9rem', fontWeight: 600 }}>Notes / Terms</label>
            ...
            <div>
                <h4 style={{ borderBottom: '1px solid var(--color-border)', paddingBottom: '8px', marginBottom: '16px' }}>Order Lines</h4>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '16px', backgroundColor: 'var(--color-background)', borderRadius: '4px', marginTop: '16px' }}>
                    <div style={{ fontWeight: 600, fontSize: '1.1rem' }}>Subtotal: {subtotal.toFixed(2)}</div>
                </div>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '24px', borderTop: '1px solid var(--color-border)', paddingTop: '16px' }}>
                ...
                <Button variant="primary" onClick={() => handleSubmit(true)} disabled={loading}>Submit Order</Button>
            </div>
        </div>
    );
};
