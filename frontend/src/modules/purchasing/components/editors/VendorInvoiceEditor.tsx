...
export const VendorInvoiceEditor: React.FC<Props> = ({ onClose, onSuccess, items, suppliers, documents }) => {
    ...
    const sourceDocs = documents.filter(d => d.document_type === 'PURCHASE_ORDER' && ['APPROVED', 'PARTIALLY_RECEIVED', 'RECEIVED'].includes(d.status));
    ...
    const handleParentChange = async (docId: string) => {
        ...
        try {
            ...
            if (doc.crm_entity) setSupplier(doc.crm_entity.toString());
            if (doc.currency) setCurrency(doc.currency);
        } catch (e) {
            console.error("Failed to fetch PO lines", e);
        }
    };
    ...
    const handleSubmit = async (submitApproval = false) => {
        ...
        try {
            ...
            if (submitApproval) {
                ...
                alert(`Vendor Invoice submitted for payment/approval!`);
            } else {
                alert(`Vendor Invoice saved as draft.`);
            }
            onSuccess();
        } catch (err: any) {
            setErrorMsg(err.response?.data?.detail || JSON.stringify(err.response?.data) || 'Failed to create Vendor Invoice.');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div style={{ backgroundColor: 'var(--color-surface)', padding: '24px', borderRadius: '8px', width: '1000px', maxWidth: '95vw', maxHeight: '90vh', overflowY: 'auto' }}>
            <h2 style={{ marginTop: 0, marginBottom: '24px', borderBottom: '1px solid var(--color-border)', paddingBottom: '12px' }}>Record Vendor Invoice</h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px', marginBottom: '24px' }}>
                ...
                <div>
                    <label style={{ display: 'block', marginBottom: '8px', fontSize: '0.9rem', fontWeight: 600 }}>Source PO / GRN</label>
                ...
                <div>
                    <label style={{ display: 'block', marginBottom: '8px', fontSize: '0.9rem', fontWeight: 600 }}>Supplier Invoice # *</label>
                    <input type="text" required value={referenceNumber} onChange={e => setReferenceNumber(e.target.value)} style={{ width: '100%', padding: '8px', border: '1px solid var(--color-border)', borderRadius: '4px' }} />
                </div>
                <div>
                    <label style={{ display: 'block', marginBottom: '8px', fontSize: '0.9rem', fontWeight: 600 }}>Invoice Date *</label>
                ...
                <div>
                    <label style={{ display: 'block', marginBottom: '8px', fontSize: '0.9rem', fontWeight: 600 }}>Due Date</label>
            ...
            <div>
                <h4 style={{ borderBottom: '1px solid var(--color-border)', paddingBottom: '8px', marginBottom: '16px' }}>Invoice Lines</h4>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '16px', backgroundColor: 'var(--color-background)', borderRadius: '4px', marginTop: '16px' }}>
                    <div style={{ fontWeight: 600, fontSize: '1.1rem' }}>Invoice Total: {subtotal.toFixed(2)}</div>
                </div>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '24px', borderTop: '1px solid var(--color-border)', paddingTop: '16px' }}>
                ...
                <Button variant="primary" onClick={() => handleSubmit(true)} disabled={loading}>Submit Invoice</Button>
            </div>
        </div>
    );
};
